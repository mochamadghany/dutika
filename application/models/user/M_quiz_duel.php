...
class M_quiz_duel extends CI_Model {
	function __construct() {
		parent::__construct();
		$this->admin = 'tb_admin';
	}

	function cek_duel_hasil($id_duel)
	{
		$p = $this->db->select('*')
						  ->from('tb_duel')
						  ->where('md5(id_duel)',$id_duel)
						  ->get()
						  ->result();
			return $p;
	}
public function ambilsoaldariquiz_1() {
			$exe1 = $this->db->select('*')
							 ->from('tb_quiz')
							 ->join('tb_quiz_language', 'tb_quiz.id = tb_quiz_language.id_quiz')
							 ->order_by('tb_quiz_language.judul_quiz','desc')
							 ->get()
							 ->result();
			return $exe1;
		}

public function ambilsoaldariquiz() {
			$exe1 = $this->db->select('*')
							 ->from('tb_quiz')
							 ->join('tb_quiz_language', 'tb_quiz.id = tb_quiz_language.id_quiz')
							 ->order_by('tb_quiz_language.judul_quiz','desc')
		...
		function lihatdaftarquiz($id_level){
		$this->db->select('*');
		$this->db->from('tb_quiz');
			$this->db->join('tb_quiz_language', 'tb_quiz.id = tb_quiz_language.id_quiz');
				$this->db->join('tb_level', 'tb_level.id_level = tb_quiz_language.id_level');
					$this->db->where('tb_level.id_level',$id_level);
		$query = $this->db->get();
